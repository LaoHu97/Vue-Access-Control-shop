        ...
        <el-form-item label="商户logo" prop="logo">
          ...
          >
            ...
            <i
              ...
              style="width: 50px;height: 50px;border: 1px dashed #d9d9d9;line-height: 50px;"
            ></i>
...
export default {
  ...
  methods: {
    returnDisplay() {
      ...
      queryMemCardByKey(para).then(res => {
        ...
        if (status == 200) {
          ...
          this.form_01.payment_is_mem =
            data.memCard.payment_is_mem == 1 ? true : false;
          if (this.form_01.payment_is_mem) {
            this.begin_time = util.formatDate.format(
              new Date(parseInt(data.memCard.base_info.begin_time) * 1000),
              "yyyy-MM-dd"
            );
            this.end_time = util.formatDate.format(
              new Date(parseInt(data.memCard.base_info.end_time) * 1000),
              "yyyy-MM-dd"
            );
          ...
          this.form_01.supply_balance =
            data.memCard.supply_balance == 2
              ? true
              : data.memCard.supply_balance == 1
              ? true
              : false;
          this.license =
            data.memCard.supply_balance == 2
          ...
          this.form_01.supply_bonus =
            data.memCard.supply_bonus == 0
              ? false
              : data.memCard.supply_bonus == 1
              ? true
              : false;
          ...
          this.form_05.promotion_url_sub_title =
            data.memCard.promotion_url_sub_title;
          this.form_05.promotion_url = data.memCard.promotion_url;
          this.form_03.business_service = JSON.parse(data.memCard.business_service).business_service
          console.log(this.form_03.business_service);
        }
      });
    },
    //动态显示
          ...
          this.$refs.form_03.validate(valid => {
            if (valid) {
              let para = {
                ...
                least_money_to_use_bonus: String(
                  this.form_01.least_money_to_use_bonus
                ), //订单满
