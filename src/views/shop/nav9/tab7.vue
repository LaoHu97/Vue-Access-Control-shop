    ...
    <el-row>
      <el-form :inline="true" :model="filters">
        ...
        <el-form-item style="float:right">
          <el-button type="primary" @click="handleAdd" round>新增卡券</el-button>
      ...
      <el-table border :data="users" highlight-current-row style="width: 100%;">
        ...
        <el-table-column label="操作" width="290">
          <template slot-scope="scope">
            ...
            <el-button
							v-if="scope.row.card_type!=='WDGIFT_COUPON'"
              :type="scope.row.status == 2 ? 'info': 'success'"
              size="mini"
							:disabled="scope.row.status==2"
              @click="uploadWinxin(scope.$index, scope.row)"
            >
  ...
  methods: {
    ...
    addSubmit: function() {
      ...
      if (card_type === 6 || card_type === 7) {
        let card_type_cope =
          this.addForm.card_type == "1"
            ? "GROUPON"
            : this.addForm.card_type == "2"
            ? "CASH"
            : this.addForm.card_type == "3"
            ? "DISCOUNT"
            : this.addForm.card_type == "4"
            ? "GIFT"
            : this.addForm.card_type == "5"
            ? "GENERAL_COUPON"
            : this.addForm.card_type == "6"
            ? "FREQUENCY"
            : this.addForm.card_type == "7"
            ? "WDGIFT_COUPON"
            : "weizhi";
        this.$router.push({
          path: "/index3/tab12-v",
