...
<section>
  ...
  <el-table :data="users" border highlight-current-row style="width: 100%;">
    ...
    <el-table-column prop="bouns" label="积分变动" min-width="120">
    ...
    <el-table-column prop="account_bouns" label="剩余积分" min-width="120">
    </el-table-column>
    ...
    <el-table-column prop="comments" label="交易类型" :formatter="reason_idformatter">
    </el-table-column>
...
export default {
  ...
  methods: {
    reason_idformatter(row, column){
      return row.reason_id === 'OLDBIND' ? '老会员绑定': row.reason_id === 'POSDEPOSIT' ? 'POS会员充值': row.reason_id === 'POSXF' ? 'POS收款': row.reason_id === 'CZLK' ? '充值领卡': row.reason_id === 'CZZS' ? '充值赠送': row.reason_id === 'CZ' ? '充值' : row.reason_id === 'XF' ? '消费' : row.reason_id === 'REFUND' ? '退款' : row.reason_id === 'TG' ? '团购' : row.reason_id === 'PT' ? '拼团' : row.reason_id === 'DH' ? '积分兑换' : '未知'
    },
    codeClick() {
      sendVerCode({
    ...
    getList() {
      ...
      queryMemBounsList(para).then((res) => {
        this.total = res.data.total;
        this.users = res.data.bonTransList;
